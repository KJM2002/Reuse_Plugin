...
FText AJMPrototypeQuestSubmitStation::BuildInteractionDescription(const FJMInteractionContext&) const
{
	...
	const int32 Owned = Inventory && QuestItem ? Inventory->GetItemQuantity(QuestItem) : 0;
	const int32 Reward = Progression ? Progression->GetPrototypeConfig().QuestReward : 0;
	return FText::Format(LOCTEXT("SubmitSummary", "필요: 점액 샘플 {0}개\n보유: {1}개\n보상: +{2} 덕코인"),
...
FJMPrototypeOperationResult AJMPrototypeQuestSubmitStation::PerformPrototypeInteraction(const FJMInteractionContext& Context)
{
	...
	FJMPrototypeOperationResult Result = Progression->SubmitQuest(
		JMPrototypeInventory::Resolve(Context.InstigatorActor), QuestItem, RequiredQuantity);
