---
title: "JMMonsterFramework Phase 11"
status: Current
authority: Guide
scope: "JMMonsterFramework Phase 0-11 setup and usage"
...
related:
  ...
  - "Testing/PHASE11_STALKER_ENEMY_TEST_KO.md"
---

# JMMonsterFramework Phase 11
...
- `AStalkerEnemyAIController`: 실제 Sight Target과의 2D 거리를 분류하고 경계 히스테리시스를 유지하는 Stalker
- `BP_StalkerEnemy`: 기존 Enemy 몸체·Sight·Navigation을 재사용하는 실제 Stalker 배치 프리셋
- `ST_StalkerEnemy`: `Approach`, `HoldDistance`, `Retreat`와 기존 Predictive Sight Lost 흐름을 사용하는 Stalker StateTree
...
Stalker는 `BP_StalkerEnemy`를 NavMesh 위에 배치한다. 실제 Sight로 Player를 발견하면 750uu보다 멀 때 접근하고, 450~750uu 선호 거리에서는 멈추며, 450uu보다 가까우면 Player를 바라본 채 NavMesh에 투영된 위치로 후퇴한다. 75uu 히스테리시스로 거리 경계 진동을 막고 Sight Lost 시 기존 Predictive Tracking → Investigate → Search → Patrol로 이어진다. Watcher Gaze는 Stalker Gameplay에 필요하지 않아 결합하지 않는다.

Encounter 단계, Flee/Hide/ReApproach, Enrage/Frenzy, Stalker Gaze 반응, 복잡한 소음 단계/점수, Suspicion, Combo, Ability/Animation Framework와 장기·복수 대상 Memory Framework는 포함되지 않는다.
